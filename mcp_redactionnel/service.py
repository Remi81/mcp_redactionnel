...
def load_prompts(path: str = None):
    global _prompts
    if _prompts is None:
        path = path or (Path(__file__).parent / 'prompts.yaml')
...
def redaction(provider: BaseProvider, sujet: str, sources: list | None = None, meta: dict | None = None, format: str = 'text') -> str:
    """Generate a redaction. If format=='html', instruct the model to return accessible HTML fragment."""
    prompts = load_prompts()
    template = prompts.get('redaction', "Rédige un texte sur: {{ sujet }}. Sources: {{ sources }}")
...
def redaction_by_name(provider_name: str, sujet: str, sources: list | None = None, meta: dict | None = None, config_path: str = 'config.yaml', format: str = 'text') -> str:
...
def mise_en_forme(provider: BaseProvider, texte: str) -> str:
    prompts = load_prompts()
    template = prompts.get('mise_en_forme', "Formate le texte suivant en HTML accessible: {{ texte }}")
...
def mise_en_forme_by_name(provider_name: str, texte: str, config_path: str = 'config.yaml') -> str:
    settings = load_settings(config_path)
    pm = ProviderManager(settings)
    prov = pm.get(provider_name)
